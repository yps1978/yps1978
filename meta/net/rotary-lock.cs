...
namespace net
{
    class ProgramB
    {
        ...
        {
            var a = new ProgramB();
            Console.WriteLine(a.getMinCodeEntryTime(3, 3, new[] {1, 2, 3}));
            Console.WriteLine(a.getMinCodeEntryTime(10, 4, new[] {9, 4, 4, 8}));
            Console.WriteLine(a.getMinCodeEntryTime(5, 1, new[] {2}));
        }
        ...
        public long getMinCodeEntryTime(int N, int M, int[] C)
        {
            ...
            for (var i = 0; i < M; i++)
            {
                var delta = Math.Abs(C[i] - pointer);
                seconds += Math.Min(delta, N - delta);
